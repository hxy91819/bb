...
- **Fork implementation:** [fix/sidebar-touch-navigation](https://github.com/hxy91819/bb/tree/fix/sidebar-touch-navigation) at [`4a0b74603`](https://github.com/hxy91819/bb/commit/4a0b74603c8a1d502e86280cf541b833156e492a), packaged as [`1b6f40111`](https://github.com/hxy91819/bb/commit/1b6f40111968f9ceb7c610d14d42f93a7355d67c). The previous packaged source [`9a0fe7b73`](https://github.com/hxy91819/bb/commit/9a0fe7b73758ab43c91b67b6dd531e3af524b9bc) remains the navigation-versus-rename patch.
...
- **Integration status:** packaged onto `local/aggregate` as an incremental patch on the `desktop-v0.43.3` baseline. Frozen as train `2026-09-21-desktop-v0.43.3`. This pass does not replace the running service.
...
- **Follow-up repair:** [fix/side-chat-main-composer](https://github.com/hxy91819/bb/tree/fix/side-chat-main-composer) at [`e3b30935a`](https://github.com/hxy91819/bb/commit/e3b30935a506755cb8913f91908b247106ea3071), packaged as [`5e0503dee`](https://github.com/hxy91819/bb/commit/5e0503deee1dccdfeafb28c6a5940b1b6f0b3748). The original wiring attached `sideChatCommand` to the inline queued-message composer instead of the primary composer, so `/side` never appeared in the command menu even on forkable providers. Focused prompt-area regression tests fail on the old wiring and pass after the move; autoreview run `20260922T105411Z-2b3602` (codex `gpt-5.6-terra`, xhigh) reported no accepted/actionable findings.
- **Integration status:** fork-only spec for now; no upstream issue was filed in this pass. The main-composer repair is frozen with the rest of the current `local/aggregate` tree as train `2026-09-22-desktop-v0.43.3`. The immutable candidate is that tree, not a fresh baseline replay. Deployment evidence stays outside this commit.
