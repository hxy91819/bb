...
`local/aggregate`，但实际主机路径、数据目录和网页入口只从该机器的本机
JSON 读取；Tailscale 相关步骤仅适用于配置了 Tailscale 的机器。
...
5. 使用 JSON 指定的持久 Node 可执行文件启用 Corepack、执行 pnpm install --frozen-lockfile 和 pnpm build（Node 版本以 JSON 为准，不强制特定大版本）。不要依赖交互 shell 的 nvm，也不要用 /tmp 的 Node 运行 systemd 服务。
...
7. 若本机使用 Tailscale Serve 作为网页入口：保持 Tailnet-only，代理 JSON 的 serveTarget，不要启用 Funnel。若本机不使用 Tailscale：保持既有网页入口不变。都不要把正式网页入口指到 scripts/bb-dev-app 的隔离开发端口或数据目录。
8. 验证 systemd 服务为 active、网页端口返回 HTTP 200（若使用 Tailscale，另核验 tailscale serve status 的目标正确）；对一个既有项目或会话只检查 HTTP 状态，不输出其内容。

完成时只报告：部署的 commit、服务状态、网页入口（使用 Tailscale 的机器为 Tailnet 代理目标），以及项目/会话检查的 HTTP 状态。若任一步不能安全完成，说明具体阻塞和已验证的状态。
```
